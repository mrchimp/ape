...
# Based on this: http://www.blog.everythings-beta.com/?p=108
# Python3enated and extended by Jake
#
        ...
        for tok in pattern:
            if isinstance(tok,str):
                ...
                if tok == 0 :
                    ...
                    raise Exception("Ints can only be 1 or 0, not:").with_traceback(tok)
                ...
                if tok[0] == 0 :
                    ...
                    raise Exception("Valid first elems of BL pattern = 1 or 0, not:").with_traceback(tok)
        ...
        for i in range(len(bindingList)):
            if isinstance( bindingList[i], list):
                for j in range(len(bindingList[i])):
                    if j > 0:
                        if bindingList[i][j].lower() in self.pronounMap:
        ...
        for elem in bindingList:
            ...
            if (elem[0] == self.Usernametag):
                ...
                if self.Usersname !='':
                    ...
                    self.Usersname = elem[1]
            if (elem[0] in bl) == elem[1:]:
                raise Exception('Binding list has name collision for: ').with_traceback(elem[0])
            else:
...
def talk(brain, debug = False):
    ...
    while 1:
        try:
            ...
            print('\nToodle do, you can type (quit) if you want be more pleasant.')
            break
        if len(inp.strip()) > 0 :
